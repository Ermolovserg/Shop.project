...
import { getProducts, searchProducts, getProduct, removeProduct, updateProduct, createProduct } from "../models/products.model";
...
productsRouter.post('/new-product', async (req: Request<{}, {}, IProductNew>, res: Response) => {
    try {
        const nweProduct = await createProduct(req.body);
        res.redirect(`/${process.env.ADMIN_PATH}/${nweProduct[0].product_id}`);
    } catch (e) {
        throwServerError(res, e);
    }
});
